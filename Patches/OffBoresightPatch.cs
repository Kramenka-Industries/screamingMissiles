...
namespace AIM9XMod.Patches
{
    ...
    public static class OffBoresightPatch
    {
        ...
        public static void HUDMissileState_SetHUDWeaponState_Postfix(HUDMissileState __instance, WeaponStation weaponStation)
        {
            ...
            if (field != null)
            {
                float firingAngle = Plugin.GetFiringGateAngle(__instance.name);
        ...
        /// 
        /// New full signature: AnalyzeTarget(WeaponStation, Unit, TrackingInfo, float armorTierOffset = 0.0f, float targetDistance = 1000f, bool mobile = false)
        /// Only one overload exists, so name-based [HarmonyPatch] resolution works without specifying parameter types.
        /// The prefix only declares WeaponStation (Harmony allows parameter subset).
        /// 
        /// QOL compatibility: The QOL mod's WeaponVariantModifier may create duplicate
        /// MissileDefinition objects and modify WeaponInfo.targetRequirements for custom
        /// weapon variants. This prefix reads targetRequirements at patch time (not cached),
        /// so any QOL modifications to targetRequirements are naturally picked up.
        /// </summary>
        ...
        public static void CombatAI_AnalyzeTarget_Prefix(WeaponStation weaponStation)
        {
            ...
            float offAngle = Plugin.GetOffBoresightAngle(weaponStation.WeaponInfo.name);
